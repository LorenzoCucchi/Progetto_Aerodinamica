function [X,Y,Z,p,f,M]=Geometria(b,Delta,C_r,C_t,d,N,P_0)
...
while i<=((N+1)*(M+1)-M-2)
    ...
    p.panels(f).A=[1000+Xb p.panels(f).P4(2) p.panels(f).P4(3)];
    p.panels(f).B=[Xb p.panels(f).P4(2) p.panels(f).P4(3)];
    Xc=(p.panels(f).P2(1)-p.panels(f).P1(1))/4+p.panels(f).P1(1);
    p.panels(f).D=[1000+Xc p.panels(f).P1(2) p.panels(f).P1(3)];
